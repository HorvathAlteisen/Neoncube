...
########################################################*/
const std::string iniFile = "neoncube\\neoncube.ini";
std::string styleFile = "neoncube\\";
...
WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, INT nCmdShow)
{
	...
	Hermes::ConfigToml *style = new Hermes::ConfigToml(styleFile);
		...
		if (!bPatchUpToDate)
		{
				 ...
				 ++it)
			{
				if (it->second == 0)
				{
					...
					{
						if ((FindFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) == 0 || (lstrcmpA(FindFileData.cFileName, _T(".")) != 0 && lstrcmpA(FindFileData.cFileName, _T("..")) != 0))
						{
							isDirEmpty = false;
							break;
						}
...
// @return value - none
//##########################################################################
void DelFile(LOCALPATCHLISTING *pListing, LPCTSTR item, LPCTSTR fpath, INT nIndex)
{
	int type = 8;
...
// @return value - none
//###########################################################################
void AddPatchEx(LOCALPATCHLISTING *pListing, LPCTSTR item, INT index, LPCTSTR fpath)
{
	int type = 0;
	if (lstrcmpiA(fpath, "FLD") == 0)
	{
...
int ProcessPatchLine(GrfCache *pCache, LOCALPATCHITEMCONTAINER *pItems, const std::pair<std::string /*patchName*/, int /*patchDest*/> &patchMetaPair, bool &isDirty)
{
	...
	case 8: // <del>FLD
	{
		...
		if (intendedTargetExists)
		{
			...
			else if (!realTargetIsDirectory && intendedTargetIsDirectory)
			{
...
INT WriteData(LPSTR lpszDir, FILE *hDataGrfTxt)
{
